...
SUPPORTED_EXTENSIONS = IMAGE_EXTENSIONS | MERMAID_EXTENSIONS
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--format", default="svg", help="Artifact extension: svg, mmd, or mermaid.")
    ...
    parser.add_argument(
        "--print-markdown",
        action="store_true",
        help="Print a Markdown image tag for SVG output instead of only the path.",
    )
    parser.add_argument(
        "--print-fence",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if fmt not in SUPPORTED_EXTENSIONS:
        print("error: unsupported format for v1. Use svg, mmd, or mermaid.", file=sys.stderr)
        return 2

    if args.print_markdown and args.print_fence:
        print("error: choose only one of --print-markdown or --print-fence", file=sys.stderr)
        return 2
    if args.print_markdown and fmt not in IMAGE_EXTENSIONS:
        print("error: --print-markdown only supports svg output", file=sys.stderr)
        return 2
